...
3. Confirm `VWORLD_API_DOMAIN` is valid for the registered VWorld origin, set `PUBLIC_BASE_URL` when the real public HTTPS origin is different, keep `INTERNAL_ONLY_STATIC_PATHS` covering unfinished pages, and keep `ADS_TXT_LINES` ready before AdSense site submission.
